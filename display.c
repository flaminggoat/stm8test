...
#define ILI_CHAR_WIDTH 3
#define ILI_CHAR_HEIGHT 6
#define ILI_CHAR_SPACING 1
...
void ili_set_bounds(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
{
	uint8_t buf[4];
...
void ili_draw_char(uint8_t x, uint8_t y, uint8_t scale, uint16_t color, char c)
{
	...
	ili_set_bounds(x, y, x + ILI_CHAR_WIDTH * scale - 1, y + ILI_CHAR_HEIGHT * scale - 1);

	ILI_ODR &= ~ILI_CS;

	c -= ' '; //font starts at space character

	for (y = 0; y < ILI_CHAR_HEIGHT; ++y)
	{
		for (uint8_t i = 0; i < scale; ++i)
		{
			for (x = 0; x < ILI_CHAR_WIDTH; ++x)
			{
				uint8_t col = font[c][x];
...
void ili_draw_string(uint8_t x, uint8_t y, uint8_t scale, uint16_t color, const char * str)
{
	while(*str)
	{
		ili_draw_char(x,y, scale, color, *str++);
		x += (ILI_CHAR_WIDTH + ILI_CHAR_SPACING) * scale;
	}
}
